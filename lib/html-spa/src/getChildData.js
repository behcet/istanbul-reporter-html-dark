...
function filterByText(nodes, text, parentPath) {
    ...
    for (let i = 0; i < nodes.length; i++) {
        const child = nodes[i];
        const childFullPath = (parentPath ? parentPath + '/' : '') + child.file;
        const matchesFilter = childFullPath.toLowerCase().includes(lowerText);

        if (child.children) {
            // For directories, check if any children match
            const filteredChildren = filterByText(child.children, text, childFullPath);
            if (filteredChildren.length > 0) {
                children.push({ ...child, children: filteredChildren });
            } else if (matchesFilter) {
                // Directory name itself matches
                children.push(child);
            }
        } else {
            // For files, check if the path matches
            if (matchesFilter) {
                children.push(child);
            }
        }
...
module.exports = function getChildData(
    ...
    if (filterText) {
        childData = filterByText(childData, filterText);
    }
